...
- (instancetype)init{
    if (self = [super init]) {
        ...
        [self.flagImage mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.mas_equalTo(self.headImage.mas_centerX).offset(WidthRate(10));
            make.height.mas_equalTo(WidthRate(16));
            make.width.mas_equalTo(WidthRate(16));
            make.centerY.mas_equalTo(self.headImage.mas_centerY).offset(WidthRate(10));
        }];
...
- (void)setData:(YHUserModel *)model{
    ...
    self.timelabel.text = [model.IdentityCategorysName componentsJoinedByString:@"、"];
