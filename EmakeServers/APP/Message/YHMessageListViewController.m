...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self getStoreDataWithRequestType:2];
...
- (void)getStoreDataWithRequestType:(NSInteger)type{
    [[YHJsonRequest shared] getAppCustomerPlatformDataWithRequestType:type SuccessBlock:^(NSDictionary *dict) {
        ...
        if ([dict objectForKey:@"AgentAuditNum"]) {
            NSNumber *AuditUserNum = dict[@"AgentAuditNum"];
    ...
    if (indexPath.section == 0) {
        ...
        cell.responseBlock = ^(SDChatMessage *msg) {
            ...
            if (!msg.staffName||msg.staffName.length<=0) {
                vc.userName = [NSString stringWithFormat:@"用户%@",[msg.phoneNumber substringFromIndex:msg.phoneNumber.length-4]];
            }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 1) {
        YHMessageListTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        ...
        vc.phoneNumber = msg.phoneNumber;
        vc.titleName = cell.nameLabel.text;
        [self.navigationController pushViewController:vc animated:YES];
    }
}
