...
- (instancetype)initWithFrame:(CGRect)frame
{
  ...
  if (self) {
      ...
      self.timeView.textColor = ColorWithHexString(@"90A6C4");
      self.timeView.font = [UIFont systemFontOfSize:12];
      ...
      title.text = @"订单信息";
      title.font = SYSTEM_FONT(AdaptFont(14));
      ...
      self.itemParameter.font = SYSTEM_FONT(AdaptFont(15));
      ...
      self.PriceLabel.font = SYSTEM_FONT(AdaptFont(15));
      ...
      [self.PriceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
          ...
          make.bottom.mas_equalTo(HeightRate(-30));
      }];
...
- (void)setData:(NSString *)eventText{
    NSDictionary *resultDic = [eventText mj_JSONObject];
    ...
    self.PriceLabel.text =  [NSString stringWithFormat:@"共%@件商品     合计：¥%@",contract.ContractQuantity,[Tools getHaveNum:contract.ContractAmount.doubleValue]];
    
}
- (NSArray *)dealWithData:(NSDictionary *)dict {
    NSArray *shoppingCartArr = [NSArray array];
    shoppingCartArr = [YHShoppingCartModel mj_objectArrayWithKeyValuesArray:dict[@"Data"]];
    return shoppingCartArr;
}
- (void)tapBubble{
    if ([self.delegate respondsToSelector:@selector(didTapMessageBubbleWithModel:)]) {
        [self.delegate didTapMessageBubbleWithModel:self.eventText];
    }
}
@end
