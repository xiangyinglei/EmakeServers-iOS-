...
-(void)initMessageChatDataWithUserID:(NSString *)userID{
    ...
    //确定执行语句
    NSString *sqlExeString = [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@(id integer PRIMARY KEY AUTOINCREMENT, sendTime text, msgType text, msgID text,msgUUID text, sender text,userId text,msg text,userName text,userPhone text, userAvata text)",filterSqlName];
...
- (void)initMessageChatListData{
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        //创建表
        BOOL createTableResult = [db executeUpdate:@"CREATE TABLE IF NOT EXISTS UserListNew(id integer PRIMARY KEY AUTOINCREMENT,userId text,userName text,userPhone text, userAvata text, userType text, sendTime text, message text, messageType text, messageCount text, storeInfo text)"];
        if (createTableResult){
            NSLog(@"创建表成功");
        }else{
            NSLog(@"创建表失败");
...
- (BOOL)userIsExist:(NSString *)userId{
    //确定执行语句
    NSString *sqlExeString = [NSString stringWithFormat:@"SELECT COUNT(userId) AS countNum FROM UserListNew WHERE userId = '%@'",userId];
    __block NSInteger count = 0;
    [self.queue inDatabase:^(FMDatabase *db) {
...
- (void)updateUserMessageCount:(NSInteger)count withUserId:(NSString *)userId{
    //确定执行语句
    //修改“messageCount”字段
    NSString *sql1 = [NSString stringWithFormat:@"UPDATE UserListNew SET %@='%@' WHERE userId = '%@'",@"messageCount",[NSString stringWithFormat:@"%ld",count],userId];
    [self.queue inDatabase:^(FMDatabase *db) {
...
- (void)addUserList:(SDChatMessage *)message andUserID:(NSString *)userId andMessageCount:(NSInteger)count{
    ...
    //确定执行语句
    NSString *sqlExeString = [NSString stringWithFormat:@"INSERT INTO UserListNew(userId,userName,userPhone,userAvata,userType,sendTime,message,messageType,messageCount,storeInfo) VALUES('%@','%@','%@','%@','%@','%@','%@','%@','%@','%@')",userId,message.staffName,message.phoneNumber,message.staffAvata,message.staffType,message.sendTime,message.msg,message.msgType,[NSString stringWithFormat:@"%ld",count],message.group];
    [self.queue inDatabase:^(FMDatabase *db){
        [db executeUpdate:sqlExeString];
    }];
}
- (NSString *)getUserInfoFromUserList:(NSString *)userId{
    NSString *sqlExeString = [NSString stringWithFormat:@"SELECT * FROM UserListNew where userId= '%@'",userId];
...
- (NSString *)getStoreInfoFromUserList:(NSString *)userId{
    NSString *sqlExeString = [NSString stringWithFormat:@"SELECT * FROM UserListNew where userId= '%@'",userId];
    __block NSString *userInfoString = nil;
    [self.queue inDatabase:^(FMDatabase *db){
        FMResultSet *res = [db executeQuery:sqlExeString];
...
- (void)addMessage:(SDChatMessage *)message andUserID:(NSString *)userId{
    ...
    NSString *filterSqlName = [sqlName stringByReplacingOccurrencesOfString:@"-" withString:@""];
    //确定执行语句
    NSString *sqlExeString = [NSString stringWithFormat:@"INSERT INTO %@(sendTime,msgType,msgID,msgUUID,sender,userId,msg,userName,userPhone,userAvata) VALUES('%@','%@','%@','%@','%@','%@','%@','%@','%@','%@')",filterSqlName,message.sendTime,message.msgType,message.msgID,message.messgaeUUID,message.sender,userId,message.msg,message.staffName,message.phoneNumber,message.staffAvata];
    [self.queue inDatabase:^(FMDatabase *db) {
        [db executeUpdate:sqlExeString];
    }];
}
...
- (void)getAllMessageWithPage:(NSInteger)page andLastMessageID:(NSInteger)lastMessageID andWithUserId:(NSString *)userId  success:(void (^)(NSArray * responseObject))success failure:(void (^)(NSString * errorObject))failure{
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        ...
        while ([res next]) {
            ...
            Message.messgaeUUID = [res stringForColumn:@"msgUUID"];
...
- (NSMutableArray *)getAllUserChatList{
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        FMResultSet *res = [db executeQuery:@"SELECT * FROM UserListNew"];
        while ([res next]) {
            SDChatMessage *Message = [[SDChatMessage alloc] init];
            Message.sendTime = [res stringForColumn:@"sendTime"];
            Message.staffType = [res stringForColumn:@"userType"];
...
- (void)deleteUserList:(NSString *)userId{
    ...
    NSString *sqlExeString = [NSString stringWithFormat:@"DELETE FROM UserListNew WHERE userId = '%@'",userId];
    [self.queue inDatabase:^(FMDatabase *db){
        [db executeUpdate:sqlExeString];
    }];
}
