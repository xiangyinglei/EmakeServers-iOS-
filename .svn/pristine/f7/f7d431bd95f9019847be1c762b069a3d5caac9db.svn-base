...
-(void)loginVIewWith:(UIView *)BackgroudView{
    ...
    [self logintext:view placeholdName:@"请输入账号" top:13 image:@"login_phone number"];
   ...
   _fogetBtn  = [UIButton buttonWithType:UIButtonTypeSystem] ;
    ...
    _fogetBtn.frame = CGRectMake(WidthRate(155),HeightRate(535), WidthRate(80), HeightRate(25));
...
-(void)logintext:(UIView *)view placeholdName:(NSString *)name top:(CGFloat)top image:(NSString *)imageName{
    ...
    [lable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(HeightRate(67));
    ...
    if ([name isEqualToString:@"请输入账号"]) {
        _textPhone = textPutIn;
        if ([[NSUserDefaults standardUserDefaults] objectForKey:LOGIN_USERNAME]) {
            _textPhone.text=[[NSUserDefaults standardUserDefaults] objectForKey:LOGIN_USERNAME];
        }
    }else{
