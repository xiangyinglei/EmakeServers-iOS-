...
+ (YBPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth messgaeCount:(NSInteger)messgaeCount delegate:(id<YBPopupMenuDelegate>)delegate
{
    ...
    popupMenu.titles = titles;
    popupMenu.images = icons;
    popupMenu.messageCount = messgaeCount;
    popupMenu.itemWidth = itemWidth;
    popupMenu.delegate = delegate;
    [popupMenu show];
    return popupMenu;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 1) {
        if (self.messageCount>0) {
            ...
            [cell.imageView showBadgeWithStyle:WBadgeStyleNumber value:self.messageCount animationType:WBadgeAnimTypeNone];
            cell.imageView.badgeCenterOffset = CGPointMake(WidthRate(25), HeightRate(5));
        }else{
