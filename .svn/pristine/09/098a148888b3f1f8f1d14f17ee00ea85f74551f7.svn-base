...
#pragma mark - UI
- (void)configData {
    if (self.selectedCount <= 0) [self setSelectedCount:6];
    if (self.preViewCount <= 0) self.preViewCount = 6;
...
- (void)refreshDoneStatus {
    if ([SuPhotoCenter shareCenter].selectedPhotos.count > 0) {
        ...
        [self.doneBtn setBackgroundColor:ColorWithHexString(APP_THEME_MAIN_COLOR)];
